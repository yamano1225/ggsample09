...
int main(int argc, const char * argv[])
{
  ...
  // プログラムオブジェクトの作成
  GLuint program = createProgram("simple.vert", "pv", "simple.frag", "fc");
  
  // in (attribute) 変数のインデックスの検索（見つからなければ -1）
  GLint nvLoc = glGetAttribLocation(program, "nv");
  GLint tvLoc = glGetAttribLocation(program, "tv");
  ...
  GLint dmapLoc = glGetUniformLocation(program, "dmap");
  GLint smapLoc = glGetUniformLocation(program, "smap");
  GLint nmapLoc = glGetUniformLocation(program, "nmap");
  ...
  ggLoadImage("diffuse.tga", GL_RGB);
  
  // 二つ目のテクスチャ
  glBindTexture(GL_TEXTURE_2D, tex[1]);
  ggLoadImage("specular.tga", GL_RGB);
  
  // 三つ目のテクスチャ (法線マップ)
  glBindTexture(GL_TEXTURE_2D, tex[2]);
  ggLoadHeight("height.tga", 5.0f);
